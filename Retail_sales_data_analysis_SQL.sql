...
ALTER TABLE order_product_mapping ADD PRIMARY KEY (order_id, product_id);
...
ALTER TABLE product_info MODIFY COLUMN product_name VARCHAR(100);
...
-- ADDING FOREIGN KEY TO SALES_FACT TABLE AND ORDER_PRODUCT_MAPPING TABLE 
...
-- ADDING FOREIGN KEY TO SALES FACT AND CUSTOMER_INFO TABLE
...
-- ADDING FOREIGN KEY TO SALES FACT AND BRANCH_DETAILS TABLE
...
-- ADDING FOREIGN KEY TO ORDER_PRODUCT_MAPPING AND PRODUCT_INFO TABLE
...
-- Preparing a table containing gender, customer_id, order_id for better analysis.
SELECT order_id , s.customer_id, gender FROM sales_fact s
INNER JOIN customer_info c  ON s.customer_id = c.customer_id;

-- transactions have been done by each gender. 
SELECT gender, count(order_id) AS num_Tranx FROM
	(SELECT order_id , s.customer_id, gender FROM sales_fact s
	INNER JOIN customer_info c  ON s.customer_id = c.customer_id) AS DATA
GROUP BY gender;

-- OR ----------

SELECT gender, COUNT(order_id) AS num_transactions
FROM sales_fact s
INNER JOIN customer_info c ON s.customer_id = c.customer_id
GROUP BY gender;
...
-- finding a table containing customers who ordered from various stores.
select customer_id, count(distinct store_id)
from sales_fact
group by customer_id;

-- customers have placed an order from more than one store
SELECT COUNT(customer_id) FROM
	(SELECT customer_id, COUNT(DISTINCT store_id) 
	FROM sales_fact
	GROUP BY customer_id
	HAVING COUNT(DISTINCT store_id) > 1) AS Distinct_store ;


-- Write the query to find the city with the highest number of orders, along with the number of orders.

SELECT city, count(order_id) AS total_orders FROM sales_fact s
INNER JOIN branch_details b ON s.store_id = b.store_id
GROUP BY city
...
SELECT area, total_sales FROM
  (SELECT area, SUM(sale_price * quantity_ordered) AS total_sales 
	FROM sales_fact f 
    INNER JOIN branch_details b ON f.store_id = b.store_id
    INNER JOIN order_product_mapping o ON f.order_id = o.order_id
    INNER JOIN product_info AS p ON o.product_id = p.product_id
    GROUP BY area
    ORDER BY total_sales DESC) AS New_set ;

-- Query to find the state with the highest number of customers.

SELECT state, num_of_customers FROM 
	( SELECT state , COUNT( DISTINCT customer_id) as num_of_customers
		FROM sales_fact s
        INNER JOIN branch_details b ON s.store_id = b.store_id
        GROUP BY state
        ORDER BY num_of_customers DESC) AS new_tab;
